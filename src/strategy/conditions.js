...
class Conditions {
  ...
  // ─────────────────────────────────────────────
  isStrongImpulse(zone) {
    const ratio = zone.impulseSize / zone.atr;
    const pass  = ratio >= config.strategy.atrMultiplier;
    return {
      pass,
      reason: pass
        ? `✅ C2 PASS: Impulse = ${ratio.toFixed(2)}× ATR (≥ ${config.strategy.atrMultiplier}×)`
  ...
  // ─────────────────────────────────────────────
  isRRSufficient(zone, currentPrice, nextResistance) {
    let sl, tp, risk, reward;

    if (zone.type === 'demand') {
      sl     = zone.low * (1 - config.strategy.zoneBuffer);
      tp     = nextResistance;
      risk   = currentPrice - sl;
      reward = tp - currentPrice;
    } else {
      sl     = zone.high * (1 + config.strategy.zoneBuffer);
    ...
    const pass = rr >= config.strategy.minRiskReward;
    ...
    return {
      ...
      reason: pass
        ? `✅ C4 PASS: R:R = 1:${rr.toFixed(2)} (≥ 1:${config.strategy.minRiskReward})`
  ...
  hasVolumeSpike(candles) {
    ...
    const pass    = ratio >= config.strategy.volumeSpikeRatio;

    return {
      pass,
      ratio: ratio.toFixed(2),
      reason: pass
        ? `✅ C6 PASS: Volume spike = ${ratio.toFixed(2)}× avg (≥ ${config.strategy.volumeSpikeRatio}×)`
  ...
  // ─────────────────────────────────────────────
  isClearPathToTP(zone, currentPrice, allZones) {
  ...
  isDailyLossOk(dailyPnL) {
    const maxLoss = config.risk.capital * (config.risk.maxDailyLossPct / 100);
  ...
  isPositionSizeValid(currentPrice, sl) {
    const riskAmt  = config.risk.capital * (config.risk.maxRiskPerTradePct / 100);
    const riskPerShare = Math.abs(currentPrice - sl);
    const qty      = Math.floor(riskAmt / riskPerShare);
    const cost     = qty * currentPrice;
    const pass     = qty >= 1 && cost <= config.risk.capital;
  ...
  runAllConditions({ zone, candles, htfCandles, currentPrice,
    ...
    const c2 = this.isStrongImpulse(zone);
    const c3 = this.isHTFAligned(zone, htfCandles);
    const c4 = this.isRRSufficient(zone, currentPrice, nextResistance);
    const c5 = this.hasConfirmationCandle(candles, zone);
    const c6 = this.hasVolumeSpike(candles);
    const c7 = this.isClearPathToTP(zone, currentPrice, allZones);
